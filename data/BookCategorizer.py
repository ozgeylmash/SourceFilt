from data.utils.lower_title import lower
import pandas as pd
...
class BookCategorizer():
    ...
    def determine_category(name, publisher):
        ...
        try:
            ...
            final_subject = "genel"
        try:
            final_grade = max(set(possible_grades), key=possible_grades.count)
        except:
            final_grade = "lise"
        try:
            final_year = max(possible_years)
        except:
            final_year = None
        try:
            final_type = max(set(possible_types), key=possible_types.count)
        except:
            final_type = "diğer"
