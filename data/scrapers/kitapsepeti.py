...
cursor.execute("TRUNCATE TABLE kitapsepeti")

for i in range(20): # range(88)
    try: 
    ...
    for book in books:
        ...
        sql = "INSERT INTO kitapsepeti (name, publisher, number_of_page, current_price, original_price, quantity, score, subject, grade, year, link, image) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        val = (name, publisher, number_of_page, current_price, original_price, quantity, score, subject, grade, year, link, image)
        cursor.execute(sql, val)


cursor.execute("SELECT name FROM kitapsepeti GROUP BY name HAVING count(*) >= 2")
...
if duplicates:
    for duplicate in duplicates:
        cursor.execute("SELECT kitapsepeti_id, quantity FROM kitapsepeti WHERE name = %s", (duplicate, ))
    ...
    sql = "DELETE FROM kitapsepeti WHERE name IN (%s)" % duplicates_string + "AND kitapsepeti_id NOT IN (%s)" % best_instances_string
    val = duplicates + best_instances
    cursor.execute(sql, val)

    cursor.execute("ALTER TABLE kitapsepeti DROP COLUMN kitapsepeti_id");
    cursor.execute("ALTER TABLE kitapsepeti ADD kitapsepeti_id INT PRIMARY KEY AUTO_INCREMENT FIRST");
