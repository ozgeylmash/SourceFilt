...
if duplicates:
    for duplicate in duplicates:
        cursor.execute("SELECT id, quantity FROM kitapsepeti WHERE name = %s", (duplicate, ))
    ...
    sql = "DELETE FROM kitapsepeti WHERE name IN (%s)" % duplicates_string + "AND id NOT IN (%s)" % best_instances_string
    val = duplicates + best_instances
    cursor.execute(sql, val)

    cursor.execute("ALTER TABLE kitapsepeti DROP COLUMN id")
    cursor.execute("ALTER TABLE kitapsepeti ADD id INT PRIMARY KEY AUTO_INCREMENT FIRST")
