...
if duplicates:
    for duplicate in duplicates:
        cursor.execute("SELECT id, quantity FROM kitapyurdu WHERE name = %s", (duplicate, ))
    ...
    sql = "DELETE FROM kitapyurdu WHERE name IN (%s)" % duplicates_string + "AND id NOT IN (%s)" % best_instances_string
    val = duplicates + best_instances
    cursor.execute(sql, val)

    cursor.execute("ALTER TABLE kitapyurdu DROP COLUMN id")
    cursor.execute("ALTER TABLE kitapyurdu ADD id INT PRIMARY KEY AUTO_INCREMENT FIRST")
